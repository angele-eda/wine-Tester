(() => {
  ...
  function renderLanguage(){
    ...
    if(separateResults.length)ui.downloadCount.textContent=convertedCountLabel(separateResults.length,separateResults.reduce((sum,item)=>sum+item.blob.size,0));
  }
  ...
  function convertedCountLabel(count,size){const total=formatBytes(size);return lang==="ko"?`변환된 파일 ${count}개 · ${total}`:lang==="ja"?`変換済みファイル ${count}個 · ${total}`:lang==="es"?`${count} archivos convertidos · ${total}`:`${count} converted files · ${total}`}
  ...
  ui.convert.addEventListener("click",async()=>{if(!files.length)return;clearResult();ui.convert.disabled=true;ui.status.textContent=copy[lang].processing;try{const result=await convert();resultUrl=URL.createObjectURL(result.blob);if(result.items){separateResults=result.items.map(item=>({...item,url:URL.createObjectURL(item.blob)}));ui.downloadCount.textContent=convertedCountLabel(result.items.length,result.items.reduce((sum,item)=>sum+item.blob.size,0));ui.downloadZip.href=resultUrl;ui.downloadZip.download=result.name;ui.downloadChoices.hidden=false;ui.status.textContent=""}else{ui.download.href=resultUrl;ui.download.download=result.name;ui.download.hidden=false;ui.status.textContent=`${copy[lang].ready} · ${formatBytes(result.blob.size)}`;ui.status.className="status success"}ui.convert.classList.add("completed")}catch(error){console.error(error);showError(copy[lang].failed)}finally{ui.convert.disabled=false}});
